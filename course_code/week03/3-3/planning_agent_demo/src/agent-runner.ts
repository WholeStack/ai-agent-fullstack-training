...
import { TaskRecorder, type TaskRecord } from "./task-record.js";
...
export async function runPlanningAgent(
  ...
  if (decided) {
    ...
    if (decided.decision === "reject") {
      ...
      await applyApprovedPatch(flowDeps, {
        actionId: archivedPending.actionId,
        by: decided.by,
        reason: decided.reason,
  ...
  // 人工拒绝：补丁不执行，也不启动模型——决定一旦做出，任务就停在这里等人工处置。
  if (!operatorRejected) {
    if (resumeEntry === "continue") {
      // 恢复：把存档里的会话历史回灌给 pi 的续跑入口，接着上次的上下文继续。
  ...
  // 运行收尾：写入终态存档（含失败/取消）。
  // 停止码 → 收尾状态只由 TaskRecorder.finish 一处决定，
  // 避免"这个停止码算不算失败"在多个文件里各写一遍。
  if (!options.disableCheckpoint) {
    if (state.stopCode?.startsWith("ABORTED")) {
      recorder.setStopCode(state.stopCode);
      recorder.transition("task_cancelled", "cancelled", state.stopCode);
    } else {
