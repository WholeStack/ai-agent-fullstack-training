...
export class EvidenceStore {
  ...
  add(input: Omit<Evidence, "id"> & { id?: string }): Evidence {
    ...
    // 还原带 ID 的证据时把计数推到已用序号之后，
    // 否则恢复后第一次分配会重新从 ev-1 开始并撞上已有证据。
    const matched = /^ev-(\d+)$/.exec(id);
...
export class PlanStore {
  ...
  /** 恢复时回填修订历史；executedSteps 从已完成/进行中步骤推算。 */
  restoreRevisions(revisions: PlanRevision[]): void {
    this.revisions.push(...structuredClone(revisions));
    this.executedSteps = this.plan.steps.filter(
  ...
  private assertVersion(expectedVersion?: number): void {    if (expectedVersion === undefined) return;
...
export class PlanningSession {
  ...
  /**
   * 从存档还原：重建 PlanStore 与证据，保留版本、修订历史、
   * 步骤状态与尝试次数（R3：恢复时保留已有进度与预算）。
   */
  restore(input: {
    plan: Plan;
    revisions: PlanRevision[];
    ...
    this.store = new PlanStore(input.plan, this.evidence, this.options);
    this.store.restoreRevisions(input.revisions);
    return this.store;
  }
