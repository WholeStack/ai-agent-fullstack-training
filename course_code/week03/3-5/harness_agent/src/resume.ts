...
export async function inspectResumeState(
  ...
  const digestFiles = input.digestFiles ?? checkpoint.payload.workspace.digestFiles ?? [
  ...
  if (digestFiles.length === 0) {
    throw new ResumeRejected("WORKSPACE_UNVERIFIABLE", "恢复缺少源码与测试文件清单，无法核查现场。");
  }
  {
  ...
  if (!journal && (input.sessionId ?? checkpoint.payload.session.sessionId)) {
    throw new ResumeRejected("JOURNAL_UNAVAILABLE", "存档引用的执行记录无法读取，不能确认是否存在未存档写入。");
  }
  if (journal) {
  ...
  for (const step of task.steps) {
    ...
    const latest = testCalls[testCalls.length - 1];
    const payload = (latest.output?.evidence as { payload?: Record<string, unknown> } | undefined)
      ?.payload;
    // 旧的完整结果不能覆盖后一次中断；passed=false 也可能只是取消标志，
    // 只有真实退出码才表示测试得到了明确结果。
    const hasCompleteResult = latest.endedAt !== undefined
      && (typeof payload?.exitCode === "number" || typeof latest.output?.exitCode === "number");
