...
async function runScripted(
  ...
  options: { maxTurns?: number; autoApprove?: boolean } = {},
) {
  ...
  const result = await runPlanningAgent({
    ...
    // 本组用例验收的是计划层（证据、修订、完成契约），改源码这一步
    // 由机器路径放行；人工审批的验收在 checkpoint-resume.test.ts 的 A9。
    ...
    const result = await runScripted([
      ...
      toolCall("write_file", {
        ...
        content: [
          ...
          "根因：只比较 UTC 日期。",
          "改动：按到期时间戳比较，到期时立即失效。",
          "验证命令：node --import tsx --test <repo>/tests/session-policy.test.ts；session-boundary.test.ts；login-flow.test.ts。",
          "结果：target、boundary、regression 均退出 0。",
          "未验证项：未做浏览器端到端与生产环境验证。",
        ].join("\n\n"),
        planStepId: "report",
      }),
      toolCall("update_plan_step", {
