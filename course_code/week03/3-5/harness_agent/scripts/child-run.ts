  ...
  return [
    ...
    tc("write_file", {
      ...
      content: [
        ...
        "",
        "## 根因",
        "只比较 UTC 日期，会话最多晚 24 小时失效。",
        "## 改动",
        "src/auth/session-policy.ts 改为比较完整时间戳，到期瞬间即失效。",
        "## 验证命令与结果",
        "修复前 run_test(scope=target) 失败；修复后 run_test 的 target、boundary、regression 三组 exit=0。",
        "实际命令为 node --import tsx --test <对应测试文件>，执行记录与内容指纹保存在 Checkpoint。",
        "## 未验证项",
        "未运行生产环境与浏览器端到端测试；本实验仅验证 fixture 登录应用。",
      ].join("\n\n"),
      planStepId: "report",
    }),
    completeStep("report"),
